...
func GetAccessToken(code string) (result *AccessTokenInfo, err error) {
	...
	defer func() {
		if r := recover(); r != nil {
			logger.Error("GetAccessToken Recovered in %v", r)
			switch x := r.(type) {
			case string:
				err = errors.NewWxClientFailedError(errors.WithMsg(x))
			case error:
				err = x
			default:
				err = errors.NewWxClientFailedError(errors.WithMsg("unknown panic"))
			}
		}
	}()
	...
	logger.Info("GetAccessToken req url: %v", url)
	resp, err := client.client.Get(url)
	if err != nil {
		logger.Error("GetAccessToken url: %v rpc err: %s", url, err.Error())
		return nil, err
	}
	defer resp.Body.Close()
	body, err := ioutil.ReadAll(resp.Body)
	if err != nil {
		logger.Error("GetAccessToken url: %v read err: %s, rsp: %v", url, err.Error(), resp.Body)
		return nil, err
	}

	rspResult := &getAccessResponse{}
	if err := json.Unmarshal(body, &rspResult); err != nil {
		logger.Error("GetAccessToken json parse error, err = %s, json = %s", err.Error(), string(body))
		return nil, err
	}

	if rspResult.Errcode != 0 {
		logger.Error("GetAccessToken error code: %v, rsp: %v", rspResult.Errcode, rspResult)
	...
	if len(rspResult.AccessToken) == 0 {
		logger.Error("GetAccessToken AccessToken is empty, rsp: %v", rspResult)
		return nil, errors.NewWxClientFailedError(errors.WithMsg("GetAccessToken AccessToken is empty"))
	}
	...
	logger.Info("GetAccessToken success, get token: %v by code: %v", result, code)
	err = saveToken(result)
	if err != nil {
		logger.Error("GetAccessToken save oid: %v token info error: %v", result.OpenID, err)
	}
...
func RefreshAccessToken(oid string) (result *AccessTokenInfo, err error) {
	...
	defer func() {
		if r := recover(); r != nil {
			logger.Error("RefreshAccessToken Recovered in %v", r)
			switch x := r.(type) {
			case string:
				err = errors.NewWxClientFailedError(errors.WithMsg(x))
			case error:
	...
	if err != nil {
		logger.Error(fmt.Sprintf("RefreshAccessToken error: %v", err.Error()))
		if err == redis.Nil {
	...
	logger.Info("RefreshAccessToken req url: %v", url)
	resp, err := client.client.Get(url)
	if err != nil {
		logger.Error("RefreshAccessToken url: %v rpc err: %s", url, err.Error())
		return nil, err
	}
	defer resp.Body.Close()
	body, err := ioutil.ReadAll(resp.Body)
	if err != nil {
		logger.Error("RefreshAccessToken url: %v read err: %s, rsp: %v", url, err.Error(), resp.Body)
		return nil, err
	}

	rspResult := &getAccessResponse{}
	if err := json.Unmarshal(body, &rspResult); err != nil {
		logger.Error("RefreshAccessToken json parse error, err = %s, json = %s", err.Error(), string(body))
		return nil, err
	}

	if rspResult.Errcode != 0 {
		logger.Error("RefreshAccessToken error code: %v, rsp: %v", rspResult.Errcode, rspResult)
	...
	if len(rspResult.AccessToken) == 0 {
		logger.Error("RefreshAccessToken AccessToken is empty, rsp: %v", rspResult)
		return nil, errors.NewWxClientFailedError(errors.WithMsg("RefreshAccessToken AccessToken is empty"))
	}
	result.AccessToken = rspResult.AccessToken
	result.RefreshToken = rspResult.RefreshToken
	result.ExpiresIn = rspResult.ExpiresIn
	result.OpenID = rspResult.OpenID
	result.Scope = rspResult.Scope
	logger.Info("RefreshAccessToken success, refresh %v by refresh_token: %v", result, refreshToken)
	err = saveToken(result)
	if err != nil {
		logger.Error("RefreshAccessToken save oid: %v token info error: %v", result.OpenID, err)
	}
...
func FetchStepsByToken(oid string, limit int8) (result map[string]int64, err error) {
	...
	defer func() {
		if r := recover(); r != nil {
			logger.Error("FetchStepsByToken recovered in %v", r)
			switch x := r.(type) {
			case string:
				err = errors.NewWxClientFailedError(errors.WithMsg(x))
			case error:
	...
	info, err := getToken(oid)
	if err != nil {
		logger.Error(fmt.Sprintf("FetchStepsByToken error: %v", err.Error()))
		if err == redis.Nil {
	...
	// get wx step
	url := formatUrl(ser.Ip(), ser.Port(), info.AccessToken, util.GetTodayBeginTime(), limit)
	// response_body：
	// {
	//   "errcode":0,
	//   "list":[
	...
	resp, err := client.client.Get(url)
	if err != nil {
		logger.Error("FetchStepsByToken url: %v rpc err: %s", url, err.Error())
		return nil, err
	}
	defer resp.Body.Close()
	body, err := ioutil.ReadAll(resp.Body)
	if err != nil {
		logger.Error("FetchStepsByToken url: %v read err: %s, rsp: %v", url, err.Error(), resp.Body)
		return nil, err
	}
	var wxSteps wxStepsH5Data
	if err := json.Unmarshal(body, &wxSteps); err != nil {
		logger.Error("FetchStepsByToken user json parse error, err = %s, json = %s", err.Error(), string(body))
		return nil, err
	}
	logger.Debug("FetchStepsByToken get rsp: %v", string(body))
	// check if no auth
	if wxSteps.Errcode != 0 {
		logger.Error("FetchStepsByToken error code: %v, json: %v", wxSteps.Errcode, string(body))
		if wxSteps.Errcode == 40001 || wxSteps.Errcode == 42001 {
	...
	logger.Info("FetchStepsByToken get oid: %v, steps: %v", oid, dateSteps)
...
func saveToken(result *AccessTokenInfo) error {
	/*
		...
		if err != nil {
			logger.Error("saveToken json marshal error: %v", err)
			return err
		}
		res, err := cache_access.RedisClient.Set(key, value, TOKEN_EXPIRE_TIME*time.Second).Result()
		if err != nil {
			logger.Error("saveToken error: %v", err)
			return err
		}
		logger.Debug("saveToken success: %v", res)
	*/
	// use mem
...
func getToken(oid string) (*AccessTokenInfo, error) {
	...
	*/
	...
	logger.Debug("getToken success: %v", accessToken)
